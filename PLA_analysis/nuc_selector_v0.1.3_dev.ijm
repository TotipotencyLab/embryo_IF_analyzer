...
// ## Image information --------------------------------------------------------
// Using H3K27ac as DNA mask for zygote seems to work well
dna_mask_ch=1;
nucleolus_mask_ch=1;
...
// So far Huang2 is pretty good for 8-cell, but not doing very well in zygote
// For Huang2 was also used for 2-cell, but one need to first crop the image to have as much cytoplasm as possible.
// The Default and Triangle methods are more promissing for zygote but still not optimal

threshold_method_nucleus = "Huang2" // default: Huang2

threshold_method_nucleolus = "Default" // default: Triangle

// sigma setting for particle detection
//	the higher the sigma value, the more smooth the particle detected
// sigma of 3 is working well for morula, higher value (e.g. 5) is better for noisy sample like zygote
sigma_nucleus = 7; // default: 8
sigma_nucleus_z = 2
sigma_nucleolus = 4; // default: 8

// size parameter
p_size_nucleus = "40-Infinity"; // default: 40-Infinity
p_size_nucleolus = "3-300"; // default: 0.50-1.00

// circularity of 1 means a perfect circle
p_circularity_nucleus = "0.00-1.00" // default: "0.60-1.00"
p_circularity_nucleolus = "0.50-1.00" // default: "0.50-1.00"

// Watershed ref: https://imagej.net/imaging/watershed
// This might come in handy for detecting nucleus of zygote
use_watershed_nucleus=false;
use_watershed_nucleus=true;
...
info_name = getInfo("slice.label");
cur_name = split(info_name, "\\/");
//Array.print(cur_name);
use_name = Array.filter(cur_name, "Position");
//out_basename=out_basename + use_name[0];
//outfile_basename = out_basename + use_name[0] + out_suffix;

// for zygote: add information about the method used for detection here
outfile_basename = out_basename + use_name[0] + "_byCH" + dna_mask_ch + "_" + threshold_method_nucleus+ out_suffix;
//out_basename=out_basename + "test"


//print(info_name);
//Array.print(cur_name);
//Array.print(use_name);
//exit();
...
// # Nucleus --------------------------------------------------------------------
clear_roi();
// mask_nucleus(dna_mask_ch, sigma, method, particle_size, particle_circularity, fill_hole, use_watershed, close_popup_window)
mask_nucleus(dna_mask_ch, sigma_nucleus, sigma_nucleus_z, threshold_method_nucleus, p_size_nucleus, p_circularity_nucleus, true, use_watershed_nucleus, close_outline_window);
...
if(n>0){
	if(extract_outline_coord){
		...
		detect_outline_coord("nucleus_outline", outdir, outfile_basename, close_outline_table_window, save_outline_coord, save_roi);
	}
	selectWindow(start_window); // come back to the main image
	if(auto_measure_results){
		measure_roi(start_window, channel_to_measure, outdir, outfile_basename+"_nucleus", auto_reset_results_window, save_measurement);
	}
	run("Select None");
}else{
...
if(run_mask_nucleoli){
	...
	//mask_nucleus(nucleolus_mask_ch, 5, "3-100", false, close_outline_window);
	//mask_nucloli(nucleolus_mask_ch, sigma, method, particle_size, particle_circularity, close_popup_window)
	mask_nucloli(nucleolus_mask_ch, sigma_nucleolus, threshold_method_nucleolus, p_size_nucleolus, p_circularity_nucleolus, close_outline_window);
	...
	if(n>0){
		...
		if(extract_outline_coord){
			detect_outline_coord("nucleolus_outline", outdir, outfile_basename, close_outline_table_window, save_outline_coord, save_roi);
		}
		selectWindow(start_window); // come back to the main image
		if(auto_measure_results){
			measure_roi(start_window, channel_to_measure, outdir, outfile_basename+"_nucleolus", auto_reset_results_window, save_measurement);
		}
...
print("Analysis Done! - " + outfile_basename);
...
function modify_roi_name_range(prefix, suffix, min_idx, max_idx){
	...
	for(i=0; i<n; i++){
		...
		if(do_rename){
			...
			roi_new_name = prefix + roi_old_name + suffix;
			// unique to this analysis, prevent the addtion of 'nucleolus' prefix to exising 'nucleus' prefix
			roi_new_name = replace(roi_new_name, "nucleolus_nucleus", "nucleus");
			// # renaming the recently added one
			roiManager("select", i); // not in original code
			roiManager("rename", roi_new_name);
...
function mask_nucleus(dna_mask_ch, sigma, sigma_z, method, particle_size, particle_circularity, fill_hole, use_watershed, close_popup_window){
...
//	run("Gaussian Blur...", "sigma="+sigma+" stack");
	run("Gaussian Blur 3D...", "X sigma=" + sigma + "Y sigma=" + sigma + "Z sigma=" + sigma_z + " stack");
...
//	run("Auto Threshold", "method=Huang2 ignore_black ignore_white white stack use_stack_histogram");
	...
	// Running watershed segmentation, this might come in handy when separaing adjacent nuclei in zygote
	...
	run("Analyze Particles...", "size="+particle_size+" circularity="+particle_circularity+" exclude include add stack");
